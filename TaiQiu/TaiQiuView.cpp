...
#define ON_RECEIVE WM_USER+6
#define BEGIN_PORT 6789
...
BEGIN_MESSAGE_MAP(CTaiQiuView, CView)
	...
	ON_WM_DESTROY()
	ON_WM_CHAR()
	ON_COMMAND(IDM_PLAYNET, OnPlaynet)
...
//		dr = mdd/BallRadius/1.2;
		...
		if(cur_desk >=0 && timetosendmove)
		{
			CString str;
			str.Format("m%c%d,", p_side, cur_desk);
			int len = str.GetLength()+3*sizeof(float);
			char * buf = new char[len];
			strcpy(buf, (LPCTSTR)str);
			float * fbuf = (float *)(buf+str.GetLength());
			fbuf[0] = hitangle;
			fbuf[1] = hitx;
			fbuf[2] = hity;
			pSock->SendMesg(buf, len, Serverpsa);
			delete [] buf;
			SetTimer(6, 1000, NULL);
			timetosendmove = FALSE;
		}
	}
...
void CTaiQiuView::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if(!bPlaying && bNewGame && cur_desk > -2)
	{
		if(cur_desk >= 0)
		{
			SendBallPos('s');
...
void CTaiQiuView::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	if(bAllStop && bPlaying)
	{
		if(Balls[0].bUP && power > 0.0f)
		{
			...
			{
				CString str;
				str.Format("h%c%d,", p_side, cur_desk);
				int len = str.GetLength()+2*sizeof(float);
				char * buf = new char[len];
				strcpy(buf, (LPCTSTR)str);
				float * fbuf = (float *)(buf+str.GetLength());
				fbuf[0] = hitangle;
...
void CTaiQiuView::NewHiting(void)
{
	BOOL bAllOver = TRUE;
	BOOL bP1Bingle = FALSE;
	BOOL bP2Bingle = FALSE;
	for(i=1; i<BALLNUM; i++)
	{
		if(Balls[i].bUP)
			bAllOver = FALSE;
		else
		{
			if(pre_bs[i])
			{
				if(i<8)			bP1Bingle = TRUE;
				else if(i>8)	bP2Bingle = TRUE;
			}
		}
	}
...
//	logindlg.DoModal();
	...
	if(m_pLoginDlg == NULL)				//Only Can Open one Window
	{
		m_pLoginDlg = new CLoginDlg(this);
		
		if(m_pLoginDlg->Create()==TRUE)
			m_pLoginDlg->ShowWindow(SW_SHOW);
	}
	else
		m_pLoginDlg->SetActiveWindow();		//Only Can Open one Window
}

void CTaiQiuView::InitConnection(void)
{
	if(pSock)
		return;

	WSADATA WSAData;
	if (WSAStartup (MAKEWORD(1,1), &WSAData) != 0)	//Init Windows Sock
	{
		return;
	}	
	...
	pSock->SetWndMesg(this, ON_RECEIVE);
	pSock->InitSock(BEGIN_PORT);
}
...
void safe_strcpy(char *des, char *src)
{
	if(src)
		strcpy(des, src);
	else
...
void CTaiQiuView::SendBallPos(char msgtype)
{
	CString str;
	str.Format("%c%c%d,", msgtype, p_side, cur_desk);
	int len = str.GetLength()+BALLNUM*2*sizeof(float);
	char * buf = new char[len];
	strcpy(buf, (LPCTSTR)str);
	float * fbuf = (float *)(buf+str.GetLength());
	for(int i=0; i<BALLNUM; i++)
	{
		fbuf[2*i] = Balls[i].xpos;
		fbuf[2*i+1] = Balls[i].ypos;
	}
	pSock->SendMesg(buf, len, Serverpsa);
	delete [] buf;
}

LRESULT CTaiQiuView::OnReceive(WPARAM wParam, LPARAM lParam)
{
	char buf[MAXBUFLEN];
	int buflen;

	CSockAddr psa = CSockAddr();
	buflen = pSock->RecvMesg(buf, MAXBUFLEN, psa);
	if(buflen < 0)	return 0L;
//	int port = psa.Port();
//	CString ip = psa.DottedDecimal();

//	CString mesg = "H,32,Haha!!!";
//	pSock->SendMesg(mesg, mesg.GetLength(), psa);
	buf[buflen] = '\0';
	switch(buf[0])
	{
	case 'L':	Serverpsa = psa;	DoLogin(buf+1);		break;
	case 'R':	DoRegister(buf+1);						break;
	case 'U':	DoUserList(buf+1);						break;
	case 'M':	DoMoreList(buf+1);						break;
	case 'C':	DoChat(buf+1);							break;
	case 'G':	DoGameSetup(buf+1);						break;

	case 's':	DoGameStart(buf+1);						break;
	case 'm':	DoMoveInfo(buf+1);						break;
	case 'h':	DoHitBall(buf+1);						break;
	case 'b':	DoBallPos(buf+1);						break;
	}
...
void CTaiQiuView::DoLogin(char * buf)
{
	switch(buf[0])
	{
	case 's':
		if(m_pLoginDlg)
		{
			userid = m_pLoginDlg->m_userid;
			m_pLoginDlg->KillTimer(0);
			m_pLoginDlg->DestroyWindow();
			m_pLoginDlg = NULL;
			//MessageBeep(MB_OK);
			PlayEffect(3);
			if(m_pPlayNetDlg == NULL)					//Only Can Open one Window
			{
				m_pPlayNetDlg = new CPlayNetDlg(this);
				
				if(m_pPlayNetDlg->Create()==TRUE)
					m_pPlayNetDlg->ShowWindow(SW_SHOW);
			}
			else
				m_pPlayNetDlg->SetActiveWindow();		//Only Can Open one Window

			m_pPlayNetDlg->m_uid = userid;
			m_pPlayNetDlg->m_info = buf+2;
			if(buf[1] == '1')
				m_pPlayNetDlg->m_info += "您的版本不是最新了，建议去下载最新版本:)\r\n";
			m_pPlayNetDlg->UpdateData(FALSE);

			m_pPlayNetDlg->OnBnClickedRefresh();
		}
		break;
	case 'f':
		if(m_pLoginDlg)
		{
			...
			if(buf[1] == '0')
				m_pLoginDlg->SetWindowText("用户不存在，请注册先！");
			else if(buf[1] == '1')
				m_pLoginDlg->SetWindowText("登录失败，密码错！");
			else if(buf[1] == '2')
				m_pLoginDlg->SetWindowText("版本不对，请去下载最新版!");
			else if(buf[1] == '3')
				m_pLoginDlg->SetWindowText("人满了，请稍后登录!");
...
void CTaiQiuView::DoRegister(char * buf)
{
	if(buf[0] == 'a')
	{
		switch(buf[1])
		...
		case 's':
			...
			{
				m_pLoginDlg->KillTimer(0);
				m_pLoginDlg->SetWindowText("注册成功！登录吧！！");
		...
		case 'f':
			if(m_pLoginDlg)
			{
				m_pLoginDlg->KillTimer(0);
				if(buf[2] == '0')
					m_pLoginDlg->SetWindowText("注册失败，联系管理员吧:(");
				else if(buf[2] == '1')
					m_pLoginDlg->SetWindowText("用户已存在，注册失败！");
				MessageBeep(MB_ICONQUESTION);
			}
			break;
		}
	...
	else if(buf[0] == 'd')
	{
		...
		case 's':
			...
			{
				m_pLoginDlg->KillTimer(0);
				m_pLoginDlg->SetWindowText("用户资料已清除，欢迎再来");
				MessageBeep(MB_ICONASTERISK);
			}
			break;
		case 'f':
			if(m_pLoginDlg)
			{
				m_pLoginDlg->KillTimer(0);
				if(buf[2] == '0')
					m_pLoginDlg->SetWindowText("清档失败，联系管理员吧:(");
				else if(buf[2] == '1')
					m_pLoginDlg->SetWindowText("用户不存在！！");
				else if(buf[2] == '2')
					m_pLoginDlg->SetWindowText("密码错！不能清档");
				MessageBeep(MB_ICONQUESTION);
			}
			break;
		}
...
int cur_w = 0;
void CTaiQiuView::DoUserList(char * buf)
{
	int i, num, dsk;
	CString str;
	char id[20], id2[20];

	if(!m_pPlayNetDlg)		return;

	if(buf[0] == 'w')
	{
		m_pPlayNetDlg->m_lobby.DeleteAllItems();
		num = safe_atoi(strtok(buf+1, ","));
		for(i=0; i<num; i++)
		{
			safe_strcpy(id,	strtok(NULL,  ","));
			m_pPlayNetDlg->m_lobby.InsertItem(i, id, 0);
		}
		cur_w = num;
	}
	else if(buf[0] == 'p')
	{
		...
		m_pPlayNetDlg->m_desk.DeleteAllItems();
		for(i=0; i<MAXPAIR; i++)
		{
			str.Format("%2d%10s:%10s", i, " ", " ");
			m_pPlayNetDlg->m_desk.InsertItem(i, str, 2);
		}

		for(i=0; i<num; i++)
		{
			dsk = safe_atoi(strtok(NULL,  ","));
			safe_strcpy(id,	 strtok(NULL,  ","));
			safe_strcpy(id2, strtok(NULL,  ","));
			str.Format("%d %10s:%10s", dsk, id, id2);
			m_pPlayNetDlg->m_desk.DeleteItem(dsk);
			m_pPlayNetDlg->m_desk.InsertItem(dsk, str, 0);
		}
	}
	else if(buf[0] == 'f')
	{
		m_pPlayNetDlg->AppendMesg("Sorry，大厅已经人满为患了!");
	}
}

void CTaiQiuView::DoMoreList(char * buf)
{
	int i, num, dsk;
	CString str;
	char id[20], id2[20];
	...
	if(buf[0] == 'w')
	{
		...
		{
			safe_strcpy(id,	strtok(NULL,  ","));
			m_pPlayNetDlg->m_lobby.InsertItem(i+cur_w, id, 0);
		}
		cur_w += num;
	}
	else if(buf[0] == 'p')
	{
		...
		for(i=0; i<num; i++)
		{
			...
			safe_strcpy(id,	 strtok(NULL,  ","));
			safe_strcpy(id2, strtok(NULL,  ","));
			str.Format("%2d %10s:%10s", dsk, id, id2);
			m_pPlayNetDlg->m_desk.DeleteItem(dsk);
			m_pPlayNetDlg->m_desk.InsertItem(dsk, str, 0);
		}
	}
...
void CTaiQiuView::DoChat(char * buf)
{
	if(!m_pPlayNetDlg)		return;

	m_pPlayNetDlg->AppendMesg(buf);
}

void CTaiQiuView::DoGameSetup(char * buf)
{
	if(!m_pPlayNetDlg)		return;

	p_side = buf[0];
	cur_desk = safe_atoi(strtok(buf+1, ","));
	m_pPlayNetDlg->OnBnClickedCancel();
	bPlaying = FALSE;
	bNewGame = TRUE;
//	NewGame();
	win_side = '1';
...
void CTaiQiuView::DoGameStart(char * buf)
{
	char tmp[10];
	memcpy(tmp, buf, 9);
	if(cur_desk != safe_atoi(strtok(tmp+1, ",")))	return;
	if(p_side == tmp[0])							return;

	if(p_side != win_side)		// 后击球方，接受对方的初始球的位置
	{
		char * ff = strstr(buf, ",");		if(!ff)		return;
		float * fbuf = (float *)(ff + 1);
		for(int i=0; i<BALLNUM; i++)
		{
			Balls[i].xpos = fbuf[2*i];
			Balls[i].ypos = fbuf[2*i+1];
		}
	}
...
void CTaiQiuView::DoMoveInfo(char * buf)
{
	char tmp[10];
	memcpy(tmp, buf, 9);
	if(cur_desk != safe_atoi(strtok(tmp+1, ",")))	return;
...
void CTaiQiuView::DoHitBall(char * buf)
{
	char tmp[10];
	memcpy(tmp, buf, 9);
	if(cur_desk != safe_atoi(strtok(tmp+1, ",")))	return;
...
void CTaiQiuView::DoBallPos(char * buf)
{
	char tmp[10];
	memcpy(tmp, buf, 9);
	if(cur_desk != safe_atoi(strtok(tmp+1, ",")))	return;
	...
	for(int i=0; i<BALLNUM; i++)
	{
		if(Balls[i].bUP)
		{
			if(fbuf[2*i+1] < -TableWidth)		// 以落袋的为标准
			{
				Balls[i].xpos = fbuf[2*i];
				Balls[i].ypos = fbuf[2*i+1];
				Balls[i].bUP = FALSE;
			...
			{
				Balls[i].xpos = (fbuf[2*i]+Balls[i].xpos)/2;
				Balls[i].ypos = (fbuf[2*i+1]+Balls[i].ypos)/2;
			}
		}
	}
